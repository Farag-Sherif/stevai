                ...
                <div>
                  <div className="tf-product-info-by-btn mb_10">
                    ...
                    <a
                      role="button"
                      onClick={() => {
                        if (isWishlistActionInProgress?.(item?.id)) return;
                        isAddedtoWishlist(item.id)
                          ? removeFromWishlist(item.id)
                          : addToWishlist(item.id);
                      }}
                      className={`box-icon hover-tooltip text-caption-2 wishlist btn-icon-action ${isAddedtoWishlist(item.id) ? "active" : ""} ${isWishlistActionInProgress?.(item?.id) ? "loading" : ""}`}
                      style={{ pointerEvents: isWishlistActionInProgress?.(item?.id) ? "none" : undefined }}
                    >
                      {isWishlistActionInProgress?.(item?.id) ? (
